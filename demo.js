...
const API_URL = "http://localhost:3001/api/connect";
  ...
  try {
    ...
    for (const store of stores) {
      ...
      entries.slice(0, 3).forEach(([key, value]) => {
        console.log(`  ${key}: ${typeof value === 'object' ? JSON.stringify(value, null, 2).slice(0, 100) + '...' : value}`);
      });
      console.log('');
    }
    ...
    console.log('\nYou can now:');
    console.log('1. Open the web UI at http://localhost:5173 (dev) or http://localhost:3001 (prod)');
